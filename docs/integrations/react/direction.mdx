...
> The `wrapper` prop above is required and must be passed a React element. The element is then
> passed a `dir` attribute and implicitly supports other environments like React Native.
