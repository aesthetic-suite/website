...
class Button extends React.Component<
  ButtonProps & WithStylesWrappedProps<'button' | 'before' | 'after'>
> {
  render() {
    const { compose: cx, children, beforeIcon, afterIcon } = this.props;
...
```tsx {2,7,10,12}
function Button({ children, selected, disabled }: ButtonProps) {
  const cx = useStyles(styleSheet);
...
```tsx {2,7-12,15,17}
function Button({ children, selected, disabled, size = 'df', palette = 'primary' }: ButtonProps) {
  const cx = useStyles(styleSheet);
...
[component style sheet](../../dev/css-in-js/style-sheets/components.md) format and instead uses
plain [style objects](../../packages/style/concepts.md#styles)! Because of this, the following
